...
const pruebas = async (req, res) => {
  try {
    const query = await db.query("SELECT * FROM departamentos");
    res.json(query.rows);
  } catch (error) {
...
const getArticulos = async (req, res, next) => {
  try {
    const allArticulos = await db.query(
      "SELECT * FROM articulos WHERE stock >= 1 ORDER BY 'idArticulo' ASC"
    );
...
const getOneArticulo = async (req, res) => {
  try {
    ...
    if (!idProducto || typeof idProducto == "string") {
      return res
        .status(404)
        .json(jsonRespuesta(404, { error: "¡Articulo no encontrado!" }));
    }

    const oneArticulo = await db.query(
...
const getArticulosHome = async (req, res) => {
  const response = await db.query(
    "SELECT * FROM public.articulos WHERE stock >= 1 ORDER BY descuento DESC, id_articulo ASC LIMIT 3"
  );
...
const putArticulos = async (req, res, next) => {
  try {
    ...
    const resultPut = await db.query(
      "UPDATE articulos SET nombre = $1 , costo = $2, margen = $3, descuento = $4, id_categoria = $5 WHERE id_articulo = $6",
      [nombre, costo, margen, descuento, id_categoria, id]
    );
