...
const autenticarValores = async (req,res,next) =>{

    const { id_usuario, id_rol , nombre, apellido, correo,  telefono , tipo_doc, contraseña } = req.body
    console.log( id_usuario, id_rol , nombre, apellido, correo,  telefono , tipo_doc, contraseña );
    ...
    try {
        ...
        //TODO: PARSEO DE DATOS QUE SE NECESITAN COMO NUMEROS
        const id_rolNumber = parseInt(id_rol);
        const telefonoNumber = parseInt(telefono);
        const idUsuarioNumber = parseInt(id_usuario);
    
        if (!id_rolNumber || !telefonoNumber) {
            return res.status(409).json(jsonRespuesta(409 , {
                error: '¡Tipo de datos incorrectos!'
            }))  
        }   
...
module.exports = { autenticarValores }